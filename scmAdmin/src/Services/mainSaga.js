...
function* superAdminUserLogin(data) {
    ...
    try {
        ...
        if (result.status == 'fail') { 
            yield put({ type: SuperAdmin_Login_Invalid, result })
        } else {
...
function* storeCompany(data) {
    ...
    try {
        ...
        if (result.status == 'fail') {
            yield put({ type: Set_Store_Company_Data_Already_Exist, result })
        } else {
...
function* getAllLevelFails() {
    try {
        let uri = API_URL.concat('/viewLevelCounts')
...
function* storeProductTemplate(data) {
    const requestData = data.data 
    try {
        let uri = API_URL.concat('/factoryAddProductTemplate')
...
function* getProductTemplate(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/viewListOfProductTemplates?factoryID=')
...
function* storeBatchTemplate(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/factoryAddBatch')
...
function* getBatchTemplate(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/viewListOfBatchesProducedByFactory?factoryID=')
        uri = uri.concat(requestData.factoryID)
...
function* getBatchDetail(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/viewBatchRecordByBatchId?batchID=')
...
function* getFactoryStatics(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/factoryStatistics?factoryID=')
        uri = uri.concat(requestData.factoryID)
...
function* getCeoStatistics(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/ceoStatistics?adminID=')
        uri = uri.concat(requestData.adminID)
        const ceoStaticsRes = yield call(Axios.get, uri)
        const result = ceoStaticsRes.data;
        yield put({ type: Set_Ceo_Statistics, result })
    } catch (error) {
        yield put({ type: Set_Ceo_Statistics, error })
        console.log("Error is ", error)
    }
}

function* getSuperAdminStatistics(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/superAdminStatistics')
        const ceoStaticsRes = yield call(Axios.get, uri)
        const result = ceoStaticsRes.data;
        yield put({ type: Set_SuperAdmin_Statistics, result })
    } catch (error) {
        yield put({ type: Set_SuperAdmin_Statistics, error })
        console.log("Error is ", error)
    }
}
...
function* sentProductListByFactory(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/viewProductsByFilter?factoryID=')
        uri = uri.concat(requestData.factoryID)
        const factorySentProductRes = yield call(Axios.get, uri)
        const result = factorySentProductRes.data;
        yield put({ type: Set_Factory_Sent_Product_List, result })
    } catch (error) {
        yield put({ type: Set_Factory_Sent_Product_List, error })
        console.log("Error is ", error)
    }
}
...
function* distributerBatchProductChartData(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/viewBatchAndProductCountForDistributor?distributorID=')
...
function* getTotalScans(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/getScans')
        const scansRes = yield call(Axios.get, uri)
        const result = scansRes.data;
        yield put({ type: Set_Total_Scan_Data, result })
    } catch (error) {
        yield put({ type: Set_Total_Scan_Data, error })
        console.log("Error is ", error)
    }
}
...
function* getAllReportForSuperAdmin(data) {
    try {
        let uri = API_URL.concat('/getAllReportForSuperAdmin')
        const scannIssueReportsRes = yield call(Axios.get, uri)
        const result = scannIssueReportsRes.data;
        yield put({ type: Set_All_Report_For_SuperAdmin, result })
    } catch (error) {
        yield put({ type: Set_All_Report_For_SuperAdmin, error })
        console.log("Error is ", error)
    }
}

function* getBatchProductForCeo(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/ceoBatchProductCovered?adminId=')
        uri = uri.concat(requestData.adminId)
        const batchProductCovered = yield call(Axios.get, uri)
        const result = batchProductCovered.data;
        yield put({ type: Set_Batch_Product_For_Ceo, result })
    } catch (error) {
        yield put({ type: Set_Batch_Product_For_Ceo, error })
        console.log("Error is ", error)
    }
}

function* getBatchProductForSuperAdmin(data) {
    try {
        let uri = API_URL.concat('/superAdminBatchProductCovered')
        const batchProductCovered = yield call(Axios.get, uri)
        const result = batchProductCovered.data;
        yield put({ type: Set_Batch_Product_For_SuperAdmin, result })
    } catch (error) {
        yield put({ type: Set_Batch_Product_For_SuperAdmin, error })
        console.log("Error is ", error)
    }
}


function* retailerBatchProductChartData(data) {
    const requestData = data.data
    try {
        let uri = API_URL.concat('/viewBatchAndProductCountForRetailer?retailerID=')
        uri = uri.concat(requestData.retailerID)
        const batchProductChartRes = yield call(Axios.get, uri)
        const result = batchProductChartRes.data;
        yield put({ type: Set_Retailer_Batch_Product_Chart, result })
    } catch (error) {
        yield put({ type: Set_Retailer_Batch_Product_Chart, error })
        console.log("Error is ", error)
    }
}
...
function* mainSaga() {
    ...
    yield takeLatest(Reset_Conpany_Data, resetConpanyData)
    ...
    yield takeLatest(Get_Ceo_Statistics, getCeoStatistics)
    yield takeLatest(Get_SuperAdmin_Statistics, getSuperAdminStatistics)
    ...
    yield takeLatest(Retailer_Batch_Product_Chart_Data, retailerBatchProductChartData)
    yield takeLatest(Get_Total_Scans, getTotalScans)
    yield takeLatest(Get_All_Report_For_Ceo, getAllReportForCeo)
    yield takeLatest(Get_Batch_Product_For_Ceo, getBatchProductForCeo)
    yield takeLatest(Get_Batch_Product_For_SuperAdmin, getBatchProductForSuperAdmin)

    yield takeLatest(Get_All_Report_For_SuperAdmin, getAllReportForSuperAdmin)
