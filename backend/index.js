...
app.post('/api/factoryAddBatch',async(req,res)=>{
  try {
    ...
    const tx =await contract.batchProduced(batchID,productIDs,companyProductIDs,batchSize,batchDescription,productTemplateID,factoryID,distributorID,factoryLocation,dataOfProduction);
    tx.wait();
    console.log("Transaction completed!");
    ...
    for(let i=0;i<array.length; i++){
      ...
      if(batchData.FactoryID==factoryID){
        ...
        result.push({
          ...
          DistributorLatLong :batchData[12],       
          AmountLeftForSellingTORetailer:batchData[13].toNumber() 
        }) 
      } 
    }  
    if(result){
...
app.post('/api/distributorScansBatch',async(req,res)=>{
  try {
    ...
    const latitudeLongitude =req.body.latitudeLongitude; 
    const tx =await contract.distributorScansBatch(batchID,distributorID,timeStamp,latitudeLongitude);
    tx.wait();
    console.log("Transaction completed!");
...
app.post('/api/retailerScansProduct',async(req,res)=>{
  try {
    ...
    const tx =await contract.retailerScansProduct(productID,retailerID,timeStamp,latitudeLongitude);
    tx.wait();
    console.log("Transaction completed!");
